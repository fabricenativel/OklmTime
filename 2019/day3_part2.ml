...
type node = {lig : int; col : int; step : int}
...
let rec make_node sx sy dir len step =
  if len = 0 then [] else 
    match dir with 
    |'R' ->  {lig  = sy; col = (sx+1); step=step} :: make_node (sx+1) sy dir (len-1) (step+1)
    |'L' ->  {lig  = sy; col = (sx-1); step=step} :: make_node (sx-1) sy dir (len-1) (step+1)
    |'U' ->  {lig  = (sy+1); col = sx; step=step} :: make_node sx (sy+1) dir (len-1) (step+1)
    |'D' ->  {lig  = (sy-1); col = sx; step=step} :: make_node sx (sy-1) dir (len-1) (step+1)
    | _ -> failwith "Bug dans les données"


let rec get_wire data sx sy step=
  match data with 
  |[] -> []
  | h :: t -> 
    ...
    let len = int_of_string (String.sub h 1 ((String.length h)-1)) in 
      match dir with
      |'R' -> (make_node sx sy dir len step)@(get_wire t (sx+len) sy (step+len))
      |'L' -> (make_node sx sy dir len step)@(get_wire t (sx-len) sy (step+len))
      |'U' -> (make_node sx sy dir len step)@(get_wire t sx (sy+len) (step+len))
      |'D' -> (make_node sx sy dir len step)@(get_wire t sx (sy-len) (step+len))
      | _ -> failwith "Bug dans les données"
...
let rec find_intersection wire1 wire2 =
  ...
  | [],_ -> []
  | _,[] -> []
  | h1::t1, h2::t2 when h1.lig=h2.lig && h1.col=h2.col -> (h1.step+h2.step)::(find_intersection t1 t2)
...
let get_input = 
  let reader = open_in filename in
    ...
    let wire1 = get_wire data1 0 0 1 in
    let line = input_line reader in 
    let data2 = String.split_on_char ',' line in
    let wire2 = get_wire data2 0 0 1 in
    ...
    print_newline (); *)
    let all_intersection = find_intersection wire1_trie wire2_trie in 
    List.fast_sort Int.compare all_intersection |> List.hd |> print_int
